    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-8 border-l-4 border-indigo-600 pl-4">
            <h1 class="font-bold text-3xl text-gray-800 flex items-center gap-2">
                ...
                <span class="text-purple-500">Buku favorit pekan ini</span>
            </h1>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
            @forelse ($favorit as $f)
                <a href="detail/{{ $f->slug }}" class="group bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition duration-300 h-full flex flex-col">
                    <div class="relative overflow-hidden">
                        @if ($f->cover)
                            <img src="{{ asset('storage/' . $f->cover) }}" alt="{{ $f->judul }}" class="w-full aspect-[3/4] object-cover group-hover:scale-105 transition duration-300">
                        @else
                            <img src="https://image.gramedia.net/rs:fit:256:0/plain/https://cdn.gramedia.com/uploads/items/9786020637389_Kisah_dari_Tebing_Buku_2_cov_FINAL-1.jpg" alt="Default Cover" class="w-full aspect-[3/4] object-cover group-hover:scale-105 transition duration-300">
                        @endif
                        <div class="absolute top-2 right-2">
                            <span class="bg-purple-600 text-white text-xs px-2 py-1 rounded-full">favorit <i class="bi bi-heart-fill "></i></span>
                        </div>
                    </div>
                    ...
                    <div class="px-4 pb-4">
                        <button class="w-full py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition">Lihat Detail</button>
                    </div>
                </a>
            @empty
                <div class="col-span-full py-16 flex flex-col items-center justify-center">
                    ...
                    <p class="text-center text-gray-400 mb-6">Silakan cek kembali nanti untuk koleksi favorit</p>
                    <a href="/" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition">Kembali ke Beranda</a>
                </div>
            @endforelse
        </div>
